...
class Pesan extends CI_Controller {
  ...
  $kategori =$this->kategori_posting_model->listing();
	$data = array ( 'title'          => 'Data Penawaran',
                 ...
                 'kategori'        =>$kategori,
        	        ...
        	        'isi'            => 'member/dasbor/pesan/list');
    $this->load->view('member/layout/wrapper',$data);
	}
  public function list_user(){
  $posting = $this->posting_model->listing();
  $pesan = $this->pesan_model->listing_user();
  $kategori =$this->kategori_posting_model->listing();
  $data = array ( 'title'          => 'Data Penawaran',
                 'posting'         => $posting,
                 'pesan'            =>$pesan,
  ...
  public function tambah($id_posting){
    ...
    $data = array ( 'title'    => 'Lakukan Penawaran Barter',
                  ...
                  'isi'      => 'member/dasbor/pesan/tambah');
      $this->load->view('member/layout/wrapper',$data);
...
public function read($id_pesan){
  ...
  $data = array ( 'title'    => $pesan->nama_barang1,
                  'pesan' =>  $pesan,
                  'kategori' =>  $kategori,
                  'isi'      => 'member/dasbor/pesan/detail');
    $this->load->view('member/layout/wrapper',$data);
}
// Delete
...
// End hapus gambar
    ...
    redirect(base_url('member/pesan'));
}
}