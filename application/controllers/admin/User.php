...
if($user->gambar != ""){
  ...
  unlink('./assets/upload/image/thumbs/'.$user->gambar);
     ...
     $this->user_model->edit($data);
      $this->session->set_flashdata('sukses','User/administrator telah diubah');
      redirect(base_url('admin/user'));
    ...
    if($this->uri->segment('4')!=$this->session->userdata('id_user'))
    {
      ...
      if ($hasil->num_rows() == 1) {
        ...
        $this->load->view('admin/layout/wrapper',$data);
          ...
          redirect(base_url('admin/user/profile_edit_password/'.$id_user));
          }else{
          $i=$this->input;
          $data=array('id_user'=>$id_user,
          'password'=>sha1($i->post('password')));
          ...
          $this->user_model->edit($data);
          redirect(base_url('admin/user/lihat_user/'.$id_user));     
        }}
      }else{
        redirect('error'); 
    }
